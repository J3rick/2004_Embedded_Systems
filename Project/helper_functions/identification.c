...
confidence_result_t chip_calculate_confidence(FlashChipData* measured, FlashChipData* expected) {
    ...
    // Original weighting per D7.2.2
    const float JEDEC_WEIGHT = 0.40;
    const float READ_WEIGHT = 0.20;
    const float WRITE_WEIGHT = 0.20;
    const float ERASE_WEIGHT = 0.10;
    const float CLOCK_WEIGHT = 0.10;
    ...
    // 5. Clock Speed Profile Match (10% weight)
    if (measured->max_clock_freq_mhz > 0 && expected->max_clock_freq_mhz > 0) {
        result.breakdown.clock_profile_available = true;
        factors_available++;
        total_weight_available += CLOCK_WEIGHT;

        float deviation = fabs(measured->max_clock_freq_mhz - expected->max_clock_freq_mhz) /
                         (float)expected->max_clock_freq_mhz;
        float normalized_score = fmax(0, 100.0 * (1.0 - deviation / 0.15));
        result.breakdown.clock_profile_score = normalized_score;
        weighted_score += CLOCK_WEIGHT * normalized_score;
    }
...
match_status_t chip_match_database(FlashChipData* test_data) {
    ...
    printf("  Weights: JEDEC 40%%, Read 20%%, Write 20%%, Erase 10%%, Clock 10%%\n");
    printf("====================================\n\n");
    ...
    for (int i = 0; i < TOP_MATCHES_COUNT; i++) {
        if (match_results[i].database_index >= 0) {
            ...
            if (match_results[i].confidence.breakdown.clock_profile_available)
                printf("     - Clock Profile (10%%): %.0f%%\n", match_results[i].confidence.breakdown.clock_profile_score);
            printf("\n");
        }
    }
